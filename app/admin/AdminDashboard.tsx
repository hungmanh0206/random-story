...
export function AdminDashboard() {
  ...
  return (
    <div className="admin-shell">
      <aside><Link className="brand admin-brand" href="/"><picture><source media="(max-width: 620px)" srcSet="/icon.jpg" /><img src="/logo-original-font.png" alt="random story." /></picture></Link><p>Trang quản trị</p><nav><button className={activeTab === "dashboard" ? "active" : ""} onClick={() => setActiveTab("dashboard")}><StageIcon name="chart" />Tổng quan</button><button className={activeTab === "posts" ? "active" : ""} onClick={() => setActiveTab("posts")}><StageIcon name="file-text" />Bài viết</button><button className={activeTab === "categories" ? "active" : ""} onClick={() => setActiveTab("categories")}><StageIcon name="layers" />Chủ đề</button><button className={activeTab === "members" ? "active" : ""} onClick={() => setActiveTab("members")}><StageIcon name="users" />Thành viên</button></nav><Link className="admin-exit" href="/"><StageIcon name="arrow-right" />Xem trang blog</Link></aside>
      <section className="admin-main">
        <header><div><h1>{activeTab === "dashboard" ? "Tổng quan thống kê" : activeTab === "posts" ? "Quản lý bài viết" : activeTab === "categories" ? "Quản lý chủ đề" : "Quản lý thành viên"}</h1></div>{(activeTab === "posts" || activeTab === "categories") && <button className="primary-btn" onClick={activeTab === "posts" ? openCreate : openCategoryCreate}>+ {activeTab === "posts" ? "Bài viết mới" : "Chủ đề mới"}</button>}</header>
        ...
        <div className="admin-card table-card admin-data-block">
          <div className="card-title"><div><h2>Tất cả bài viết</h2><small>{filteredPosts.length}/{posts.length} bài viết</small></div><div className="admin-controls"><label className="admin-search"><StageIcon name="search" /><input type="search" value={postSearch} onChange={(event) => setPostSearch(event.target.value)} placeholder="Tìm bài viết..." aria-label="Tìm bài viết" /></label><Combobox value={postCategory} onChange={setPostCategory} ariaLabel="Lọc chủ đề" options={[{ value: "all", label: "Mọi chủ đề" }, ...categories.map((item) => ({ value: item.name, label: item.name }))]} /><Combobox value={postStatus} onChange={setPostStatus} ariaLabel="Lọc trạng thái" options={[{ value: "all", label: "Mọi trạng thái" }, { value: "published", label: "Đã xuất bản" }, { value: "draft", label: "Bản nháp" }, { value: "archived", label: "Lưu trữ" }]} /><Combobox value={postSort} onChange={setPostSort} ariaLabel="Sắp xếp bài viết" options={[{ value: "latest", label: "Mới nhất" }, { value: "oldest", label: "Cũ nhất" }, { value: "az", label: "A–Z" }, { value: "za", label: "Z–A" }]} /></div></div>
          ...
          <div className="card-title"><div><h2>Tất cả chủ đề</h2><small>{filteredCategories.length}/{categories.length} chủ đề</small></div><label className="admin-search"><StageIcon name="search" /><input type="search" value={categorySearch} onChange={(event) => setCategorySearch(event.target.value)} placeholder="Tìm chủ đề..." aria-label="Tìm chủ đề" /></label></div>
          ...
          <div className="card-title"><div><h2>Tất cả thành viên</h2><small>{filteredMembers.length}/{members.length} thành viên</small></div><div className="admin-controls"><label className="admin-search"><StageIcon name="search" /><input type="search" value={memberSearch} onChange={(event) => setMemberSearch(event.target.value)} placeholder="Tìm thành viên..." aria-label="Tìm thành viên" /></label><Combobox value={memberRole} onChange={setMemberRole} ariaLabel="Lọc vai trò" options={[{ value: "all", label: "Mọi vai trò" }, { value: "admin", label: "Quản trị viên" }, { value: "reader", label: "Thành viên" }]} /></div></div>
      ...
      {formOpen && <div className="modal-backdrop" onMouseDown={() => setFormOpen(false)}><section className="post-editor" onMouseDown={(e) => e.stopPropagation()}><button className="close-btn" onClick={() => setFormOpen(false)} aria-label="Đóng"><StageIcon name="close" /></button><h2>{editing ? "Sửa bài viết" : "Bài viết mới"}</h2><form onSubmit={save}>
        <label>Tiêu đề<input required value={form.title} onChange={(e) => changePostTitle(e.target.value)} /></label>
        <div className="editor-grid"><label>Slug<input value={form.slug} onChange={(e) => setForm({ ...form, slug: e.target.value })} placeholder="Tự tạo từ tiêu đề" /></label><label>Chủ đề<Combobox value={form.category} onChange={(category) => setForm({ ...form, category })} ariaLabel="Chọn chủ đề" options={categories.map((category) => ({ value: category.name, label: category.name }))} /></label></div>
        ...
        <label>Trạng thái<Combobox value={form.status} onChange={(status) => setForm({ ...form, status: status as typeof form.status })} ariaLabel="Chọn trạng thái" options={[{ value: "draft", label: "Bản nháp" }, { value: "published", label: "Xuất bản" }, { value: "archived", label: "Lưu trữ" }]} /></label>
        {message && <p className="auth-message">{message}</p>}<button className="primary-btn">{editing ? "Lưu thay đổi" : "Tạo bài viết"}</button>
      </form></section></div>}
      {categoryFormOpen && <div className="modal-backdrop" onMouseDown={() => setCategoryFormOpen(false)}><section className="category-editor" onMouseDown={(e) => e.stopPropagation()}><button className="close-btn" onClick={() => setCategoryFormOpen(false)} aria-label="Đóng"><StageIcon name="close" /></button><h2>{editingCategory ? "Sửa chủ đề" : "Chủ đề mới"}</h2><form onSubmit={saveCategory}>
