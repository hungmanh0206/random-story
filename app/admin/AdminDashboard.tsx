    ...
    <div className="admin-shell">
      ...
      <section className="admin-main">
        ...
        <div className="admin-card table-card admin-data-block">
          <div className="card-title"><div><h2>Tất cả bài viết</h2><small>{filteredPosts.length}/{posts.length} bài viết</small></div><div className="admin-controls"><label className="admin-search"><StageIcon name="search" /><input type="search" value={postSearch} onChange={(event) => setPostSearch(event.target.value)} placeholder="Tìm bài viết..." aria-label="Tìm bài viết" /></label><label className="admin-select"><select value={postCategory} onChange={(e) => setPostCategory(e.target.value)} aria-label="Lọc chủ đề"><option value="all">Mọi chủ đề</option>{categories.map((item) => <option key={item.id} value={item.name}>{item.name}</option>)}</select><StageIcon name="chevron-down" /></label><label className="admin-select"><select value={postStatus} onChange={(e) => setPostStatus(e.target.value)} aria-label="Lọc trạng thái"><option value="all">Mọi trạng thái</option><option value="published">Đã xuất bản</option><option value="draft">Bản nháp</option><option value="archived">Lưu trữ</option></select><StageIcon name="chevron-down" /></label><label className="admin-select"><select value={postSort} onChange={(e) => setPostSort(e.target.value)} aria-label="Sắp xếp bài viết"><option value="latest">Mới nhất</option><option value="oldest">Cũ nhất</option><option value="az">A–Z</option><option value="za">Z–A</option></select><StageIcon name="chevron-down" /></label></div></div>
